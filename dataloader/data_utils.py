...
def set_up_datasets(args):
    ...
    if args.dataset == 'coco':
        import dataloader.coco.coco as Dataset
        args.base_class = 60
        args.num_classes = 80
        args.way = 10
        args.shot = 5
...
def get_base_dataloader(args):
    ...
    if args.dataset == 'coco':
        trainset = args.Dataset.COCODataset(root=args.dataroot, train=True,
                                     index=class_index, base_sess=True)
        testset = args.Dataset.COCODataset(root=args.dataroot, train=False, index=class_index)
...
def get_new_dataloader(args, session):
    ...
    if args.dataset == 'mini_imagenet':
        testset = args.Dataset.MiniImageNet(root=args.dataroot, train=False,
                                      index=class_new)
    if args.dataset == 'coco':
        testset = args.Dataset.COCODataset(root=args.dataroot, train=False,
                                    index=class_new)
